btoApp.service('manageAvatarService', function ($rootScope, utilService, $timeout, CONFIG) {    
    ...
    this.showDialogUpdateProfilePicture = function (type,dream_type_id) {
        ...
        if ($rootScope.Avatar.type === 0) {
            ...
            if ($rootScope.selectedDream.photo_path == null || $rootScope.selectedDream.photo_path == '') {
                ...
                $rootScope.Avatar.src = $rootScope.selectedDream.photo_path;            
        }
        else if ($rootScope.Avatar.type === 3) {
            $rootScope.Avatar.dream_type_id = dream_type_id;
            if ($rootScope.selectedLifeEvent.photo_path == null || $rootScope.selectedLifeEvent.photo_path == '') {
                if ($rootScope.Avatar.dream_type_id === 4)
                    $rootScope.Avatar.src = "/Themes/3/Content/Images/default-residence.jpg";
                else if ($rootScope.Avatar.dream_type_id === 5)
                    $rootScope.Avatar.src = "/Themes/3/Content/Images/default-independent.jpg";
                else if ($rootScope.Avatar.dream_type_id === 6)
                    $rootScope.Avatar.src = "/Themes/3/Content/Images/default-income.jpg";
                $rootScope.Avatar.canSave = 'false';
            }
            else
                $rootScope.Avatar.src = $rootScope.selectedLifeEvent.photo_path;            
        }
    ...
    this.setUrlToImage = function (url) {
        if (angular.isDefined(url) && $rootScope.Avatar.type != null) {
            if ($rootScope.Avatar.type == 0) {
                ...
                $rootScope.selectedDream.photo_path = url;
            }
            else if ($rootScope.Avatar.type === 3) {
                $rootScope.selectedLifeEvent.photo_path = url;
            }
    ...
    this.editThumbnail = function () {        
        ...
        $timeout(function () {
            ...
            $("img-crop").children().first().removeAttr("style");
            $("img-crop").children().first().prop({
                width: tempW,
                height: tempH
            });
        }, 500);
