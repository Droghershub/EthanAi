...
btoApp.directive('materialSelect', ['$timeout', '$interval', '$rootScope', '$filter', function ($timeout, $interval, $rootScope, $filter) {
    ...
    return {
        ...
        template: function (elem, attrs) {
            ...
            result += '<li ng-init="textsearch =\'\' "><span> <input class="form-control input-search-autocomplete" type="text" id="{{ID}}-input-search" ng-model="textsearch"  /> </span></li>';
...
btoApp.directive('mdSlider', function ($timeout, $mdGesture, $filter, $document, $interval) {
    ...
    var rangeSlideController = ['$scope', '$timeout', function ($scope, $timeout) {
        ...
        function showThumbText() {
            if ($scope.specialValue) {
                ...
                if ($scope.isShowPercent) {
                    $scope.thumbText = $filter('currency')($scope.max, '', 2) + '%';
                } else {
                    $scope.thumbText = $filter('currency')($scope.max, '', 2);
                }
            } else if ($scope.isShowPercent) {
                $scope.thumbText = $filter('currency')($scope.modelData, '', 2) + '%';
            } else {
                $scope.thumbText = $filter('currency')($scope.modelData, '', 2);
            }
        ...
        $scope.updateSignPosition = function () {
            ...
            $scope.signWidth = $scope.component.signContainer.width() + 4;
            $scope.signAfterLeft = ($scope.signWidth / 2) - 14;
            ...
            var template = '<div class="md-slider-wrapper">' +
                '<div class="md-track-container">' +
                  ...
                  '<div class="md-track-ticks"><ul>' +
                    '<li ng-show="isReachMax" class="reach_max_circle" ng-style="{left: \'100%\', \'background-color\': config.boxBackgroundColor}" ng-mouseover="specialValueHoverIn($event, \'modelData\')" ng-mouseleave="specialValueHoverOut($event, \'modelData\')"><div class="tooltip fade in top" role="tooltip"  >{{config.textChangeModelData | translate}}</div></li>' +
                    '<li ng-show="!isReachMax && isCanChangeMax" class="reach_max_circle" ng-style="{left: \'100%\', \'background-color\': config.reachMaxTrackColor}" ng-mouseover="specialValueHoverIn($event, \'maxData\')" ng-mouseleave="specialValueHoverOut($event, \'maxData\')"><div class="tooltip fade in top" role="tooltip"  >{{config.textChangeMaxData | translate}}</div></li>' +
                ...
                '<div class="md-thumb-container" >' +
                  '<div class="md-thumb"><div class="after_thumb" ng-style="{\'background-color\': isReachMax? config.reachMaxTrackColor: config.boxBackgroundColor }"></div></div>' +
                  '<div class="md-focus-thumb"></div>' +
                  '<div class="md-focus-ring"></div>' +
                  '<div class="md-sign" ng-style="{\'background-color\': isReachMax? config.reachMaxTrackColor: config.boxBackgroundColor, \'color\': isReachMax? config.reachMaxColor : config.boxColor}">' +
                    '<span class="md-thumb-text"></span>' +
                    '<div class="after" ng-style="{color: isReachMax? config.reachMaxTrackColor: config.boxBackgroundColor}"></div>' +
                  '</div>' +
                  '<div class="md-disabled-thumb"></div>' +
                '</div>' +
                '<div class="min">{{min | currency:\'\'}}<span ng-if="isShowPercent">%</span></div>' +
                '<div class="max" ng-if="!isReachMax" ng-style="{\'background-color\':config.reachMaxTrackColor , \'color\': config.reachMaxColor}">{{max | currency:\'\'}}<span ng-if="isShowPercent">%</span></div>' +
                '<div class="max" ng-if="isReachMax" ng-style="{\'background-color\':config.boxBackgroundColor, \'color\': config.boxColor}">{{modelData | currency:\'\'}}<span ng-if="isShowPercent">%</span></div>' +
