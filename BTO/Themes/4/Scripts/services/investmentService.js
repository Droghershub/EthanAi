...
btoApp.service('investmentService', function ($rootScope, utilService, $timeout, CONFIG, $filter, $http) {
    ...
    $rootScope.cardIndex = {
        'overall_investments': 0,
        'contingency_household': 2,
        'trading_household': 3,
        'bank_household': 4,
        'cpf': 1,
        'contingency_ratio': 5
    }
    $rootScope.investmentCardsIndex = {
        'overall_investments': 1,
        'contingency_household': 2,
        'trading_household': 3,
        'bank_household': 4,
        'cpf': 0,
    ...
    var chart = null;
    this.carouselIndex = $rootScope.cardIndex.overall_investments;
    ...
    this.getAllPortfolio = function () {
        var self = this;
        utilService.callApi('GET', '/api/portfolio/get', {}, {}, function (response) {
            self.portfolioData.portfolioList = response;
        });
    }
    
    this.getAllPortfolio();

    this.selectPortfolio = function (item) {
        this.portfolioData.selectedPortfolio = item;
        $rootScope.utilService.scopeApply();
    }
    this.showImportPortfolioDialog = function () {
        this.portfolioData.uploadFile = null;
        $("#import_portfolio_file").val('');
        $('#importPortfolioDialog').modal({ backdrop: 'static', keyboard: false });
    }

    this.importPorfolio = function () {
        if (this.portfolioData.uploadFile != null || this.portfolioData.uploadFile == '') {
            var self = this;
            var fd = new FormData();
            fd.append('file', this.portfolioData.uploadFile);
            $('#loading_icon').show();
            $http.post('/api/portfolio/import', fd, {
                transformRequest: angular.identity,
                headers: { 'Content-Type': undefined }
            }).success(function (response) {
                self.portfolioData.portfolioList = response;
                $rootScope.utilService.showSuccessMessage('The portfolio were imported successfully');
                $rootScope.utilService.scopeApply();
                $('#loading_icon').hide();
            }).error(function (response, header) {
                $rootScope.utilService.showErrorMessage(response[0]);
                $('#loading_icon').hide();
            });
            $('#importPortfolioDialog').modal('hide');
        } else {
            $rootScope.utilService.showErrorMessage('Please select portfolio file');
        }
    }

    this.deletePortfolioDialog = function () {
        var self = this;
        utilService.callApi('POST', '/api/portfolio/delete/' + this.portfolioData.selectedPortfolio.id, {}, {}, function (response) {
            console.log(response);
            $rootScope.utilService.showSuccessMessage('The portfolio were deleted successfully');
            self.getAllPortfolio();
            self.portfolioData.selectedPortfolio = null;
        });

    }

    this.sumPostionOfPortfolio = function () {
        var result = 0;
        if (this.portfolioData.selectedPortfolio && this.portfolioData.selectedPortfolio.portfolios.length > 0) {
            for (var i = 0; i < this.portfolioData.selectedPortfolio.portfolios.length; i++) {
                result += this.portfolioData.selectedPortfolio.portfolios[i].position;
            }
        }
        return result;
    }
    ...
    $rootScope.$watch('investment.carouselIndex', function () {
        $rootScope.investment.portfolioData.selectedPortfolio = null;
        if ($rootScope.investment.carouselIndex == $rootScope.cardIndex.overall_investments || $rootScope.investment.carouselIndex == $rootScope.cardIndex.trading_household) {
    ...
    this.selectInvestmentChoice = function (value) {
        this.portfolioData.selectedPortfolio = null;
    ...
    this.selectChangeInvestmentType = function (index) {
        if (!$rootScope.isDisabledInvestmentChoice) {
            ...
            if (cursorIndex == $rootScope.cardIndex.cpf) { // cpf 
                this.initDataForCpfInvestment();
            }

            var displayValue = $rootScope.investmentCards[$rootScope.investmentCardsIndex.cpf];
    ...
    this.saveIronReserve = function () {
        $rootScope.investmentCards[$rootScope.investmentCardsIndex.contingency_ratio].value = Math.round($rootScope.investmentCards[$rootScope.investmentCardsIndex.contingency_household].config.value) / 12;
        var data = { name: $rootScope.investmentCards[$rootScope.investmentCardsIndex.contingency_ratio].key, value: $rootScope.investmentCards[$rootScope.investmentCardsIndex.contingency_ratio].value, type: $rootScope.investment.carouselIndex };
        if (!$rootScope.playBackPlayerData.isPlaying) {
        ...
        $rootScope.SendingScreenSharingDataObject($rootScope.investmentCards[$rootScope.investmentCardsIndex.contingency_ratio].value, 'investment', 'oKDialogIronReserve', 'div_Investment');
    }
    ...
    this.showDialogIronReserve = function () {
        ...
        $timeout(function () {
            ...
            $('#CancelSaveIron').bind('click', function () {
                $('#SaveIronChange').unbind('click');
                $('#CancelSaveIron').unbind('click');
                $rootScope.SendingScreenSharingDataObject($rootScope.investmentCards[$rootScope.investmentCardsIndex.contingency_household].config.value, 'investment', 'cancelDialogIronReserve', 'div_dialogIronReserve');
            });
            $rootScope.SendingScreenSharingDataObject($rootScope.investmentCards[$rootScope.investmentCardsIndex.contingency_household].config.value, 'investment', 'showDialogIronReserve', 'div_Investment');
        }, 100);
    }
    ...
    this.initValueForInvestmentCards = function () {
        ...
        utilService.scopeApply();
        $rootScope.summaryCardService.updateInvestment();
    }
    ...
    this.updateTotalInvestment = function () {
        $rootScope.investmentCards[$rootScope.investmentCardsIndex.overall_investments].value = $rootScope.investmentCards[$rootScope.investmentCardsIndex.cpf].value + $rootScope.investmentCards[$rootScope.investmentCardsIndex.contingency_household].value + $rootScope.investmentCards[$rootScope.investmentCardsIndex.trading_household].value + $rootScope.investmentCards[$rootScope.investmentCardsIndex.bank_household].value;
    }
    ...
    this.initDataForProviderCardValue = function () {
        var data = [], objData;
        if ($rootScope.investment.carouselIndex == $rootScope.cardIndex.bank_household || $rootScope.investment.carouselIndex == $rootScope.cardIndex.contingency_household) {
    ...
    this.initDataForProvider = function () {
        ...
        if (angular.isDefined($rootScope.investmentData)) {
            if ($rootScope.isIndividualInvestment == true) {
                var indexSwiper = this.getIndexSwiperOfCardByCarouselIndex(this.carouselIndex);
                if ($rootScope.investment.carouselIndex != $rootScope.cardIndex.cpf || ($rootScope.investment.carouselIndex == $rootScope.cardIndex.cpf && $rootScope.cpfInvestmentChoice < 0)) {

                    if ($rootScope.investment.carouselIndex == $rootScope.cardIndex.cpf) {
    ...
    this.initChartForCardValue = function () {
        ...
        chart = AmCharts.makeChart("chartInvestment", {
            ...
            "graphs": [{
                "balloonText": utilService.translate("Value") + " <span style='font-size:11px; color:#000000;'><b> " + $rootScope.PersonaPlan.currency_code + " [[value]]</b></span>",
                "fillAlphas": 0.6,
                "lineAlpha": 0.4,
                "stackable": false,
    ...
    this.initChartForCPFTotal = function () {
        ...
        chart = AmCharts.makeChart("chartInvestment", {
            ...
            "graphs": [
                {
                    "balloonText": utilService.translate("cpf_oa") + " <span style='font-size:11px; color:#000000;'><b> " + $rootScope.PersonaPlan.currency_code + " [[value]]</b></span>",
                    "fillAlphas": 1,
                    "lineAlpha": 0.4,
                    "stackable": true,
                 ...
                 {
                     "balloonText": utilService.translate("cpf_ms") + " <span style='font-size:11px; color:#000000;'><b> " + $rootScope.PersonaPlan.currency_code + " [[value]]</b></span>",
                     "fillAlphas": 1,
                     "lineAlpha": 0.4,
                     "stackable": true,
                  ...
                  {
                      "balloonText": utilService.translate("cpf_sa") + " <span style='font-size:11px; color:#000000;'><b> " + $rootScope.PersonaPlan.currency_code + " [[value]]</b></span>",
                      "fillAlphas": 1,
                      "lineAlpha": 0.4,
                      "stackable": true,
                 ...
                 {
                     "balloonText": utilService.translate("cpf_ra") + " <span style='font-size:11px; color:#000000;'><b> " + $rootScope.PersonaPlan.currency_code + " [[value]]</b></span>",
                     "fillAlphas": 1,
                     "lineAlpha": 0.4,
                     "stackable": true,
    ...
    this.init = function () {
        ...
        chart = AmCharts.makeChart("chartInvestment", {
            ...
            "graphs": [{
                "balloonText": utilService.translate("Optimistic") + " <span style='font-size:11px; color:#000000;'><b> " + $rootScope.PersonaPlan.currency_code + " [[value]]</b></span>",
                "fillAlphas": 0.6,
                "lineAlpha": 0.4,
                "stackable": false,
                ...
                "balloonText": utilService.translate("Average") + " <span style='font-size:11px; color:#000000;'><b> " + $rootScope.PersonaPlan.currency_code + " [[value]]</b></span>",
                "fillAlphas": 1,
                "lineAlpha": 0.4,
                "stackable": false,
                ...
                "balloonText": utilService.translate("Pessimistic") + " <span style='font-size:11px; color:#000000;'><b> " + $rootScope.PersonaPlan.currency_code + " [[value]]</b></span>",
                "fillAlphas": 1,
                "lineAlpha": 0.4,
                "stackable": false,
    ...
    this.changeLanguageOfChart = function () {
        chart.graphs[0].balloonText = utilService.translate("Optimistic") + " <span style='font-size:11px; color:#000000;'><b> " + $rootScope.PersonaPlan.currency_code + " [[value]]</b></span>";
        chart.graphs[0].title = utilService.capitalise(utilService.translate("Optimistic"));
        chart.graphs[1].balloonText = utilService.translate("Average") + " <span style='font-size:11px; color:#000000;'><b> " + $rootScope.PersonaPlan.currency_code + " [[value]]</b></span>";
        chart.graphs[1].title = utilService.capitalise(utilService.translate("Average"));
        chart.graphs[2].balloonText = utilService.translate("Pessimistic") + " <span style='font-size:11px; color:#000000;'><b> " + $rootScope.PersonaPlan.currency_code + " [[value]]</b></span>";
    ...
    this.updateChart = function () {
        if ($rootScope.isIndividualInvestment == true) {
            ...
            } else if ($rootScope.investment.carouselIndex == $rootScope.cardIndex.contingency_household || $rootScope.investment.carouselIndex == $rootScope.cardIndex.bank_household) {
                if (this.currentChart != 'irc') {
                    this.initChartForCardValue();
                    this.currentChart = 'irc';
                } else {
    ...
    this.playBack = function (playAction) {
        ...
        if (playAction.data.variable === 'investment_choice') {
            ...
            $rootScope.investment.carouselIndex = $rootScope.cardIndex.overall_investments;
            ...
            $rootScope.investment.carouselIndex = $rootScope.cardIndex.contingency_household;
            ...
            $rootScope.investment.carouselIndex = $rootScope.cardIndex.bank_household;
            ...
            $rootScope.investment.carouselIndex = $rootScope.cardIndex.trading_household;
            ...
            $rootScope.investment.carouselIndex = $rootScope.cardIndex.cpf;
    ...
    this.UpdateControlForShareScreen = function (obj) {
        switch (obj.actionEvent) {
            ...
            case 'oKDialogIronReserve':
                $rootScope.investmentCards[$rootScope.investmentCardsIndex.contingency_ratio].value = parseFloat(obj.newValue);
                $rootScope.investmentCards[$rootScope.investmentCardsIndex.contingency_household].config.value = parseFloat(obj.newValue) * 12;
                $timeout(function () {
            ...
            case 'cancelDialogIronReserve':
                $rootScope.investmentCards[$rootScope.investmentCardsIndex.contingency_household].config.value = obj.newValue;
                $timeout(function () {
                    $('#' + obj.controlID).modal('hide');
                }, 2000);
                utilService.scopeApply();
