btoApp.service('sharingService', function ($rootScope, personalPlanService, parameterService, utilService, $timeout, $state, $locale, accountService, playbackService, CONFIG, $cookies) {
    $rootScope.viewers = [];
        ...
        if (obj.action != null && obj.action.indexOf('CHANGE_COMPONENT_') == 0) {
            ...
            switch (obj.action) {
                ...
                case 'ratingService':
                    ...
                    break;
                case 'summaryCardService':
                    $rootScope.summaryCardService.UpdateControlShareScreen(obj);
                    break;
                case 'quickSettingService':
                    $rootScope.quickSettingService.UpdateControlShareScreen(obj);
                    break;
                case 'utilService':
                    $rootScope.utilService.UpdateControlShareScreen(obj);
                    break;
                default:
    ...
    this.initSharing = function () {
        hub.client.callClientFromAdmin = function () {
            con
        }
        ...
        hub.client.updateParameter = function (target_product_version_id, parameter, defaultvalue) {
         //   console.log('updateParameter ', target_product_version_id, parameter, defaultvalue , product_version_id);
            if (product_version_id != target_product_version_id)
                return;
            if (parameter == 'persona_plan.number_trials') {
                $rootScope.PersonaPlan.number_trials = parseInt(defaultvalue);
                $rootScope.utilService.scopeApply();
            }
            else if (parameter == 'persona_plan.mc_top_value') {
                $rootScope.PersonaPlan.mc_top_value = parseFloat(defaultvalue);
                $rootScope.utilService.scopeApply();
            }
            else if (parameter == 'persona_plan.mc_bottom_value') {
                $rootScope.PersonaPlan.mc_bottom_value = parseFloat(defaultvalue);
                $rootScope.utilService.scopeApply();
            }
        };

        hub.client.updateParameterFormula = function (target_product_version_id, parameterId) {
           // console.log('updateParameterFormula', target_product_version_id, parameterId)
            if (product_version_id != target_product_version_id)
                return;
            parameterService.UpdateParameter(product_version_id, parameterId);
        };
