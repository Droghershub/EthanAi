...
btoApp.service('savingRateService', function ($rootScope, utilService, $timeout, CONFIG, $filter) {
    ...
    this.inflation_status = isCumulative_inflation;
    ...
    this.updateCashFlow = function (cashFlowData) {
        ...
        var self = this;
        var cashFlowTemp = null;
        if (angular.isDefined(cashFlowData) && cashFlowData != null) {
            if ($rootScope.isUpdateCashFlowTree) {
                cashFlowTemp = angular.copy(cashFlow);
                if ($rootScope.profile.client.married_status == null || $rootScope.profile.client.married_status == 0) {
                    this.RemoveSpouseFromCashFlow(cashFlowTemp.income);
                    this.RemoveSpouseFromCashFlow(cashFlowTemp.expense);
                    this.RemoveSpouseFromCashFlow(cashFlowTemp.cpf);
                ...
                this.RemoveChildFromCashFlow(cashFlowTemp.expense, $rootScope.profile.children.childrens.length);
                this.RemoveChildFromCashFlow(cashFlowTemp.cpf, $rootScope.profile.children.childrens.length);
                $rootScope.cashFlow = angular.copy(cashFlowTemp);
                $rootScope.isUpdateCashFlowTree = false;
            }
    ...
    this.updateYearlyCostReduction = function () {
        //console.log('updateYearlyCostReduction');
        if ($rootScope.PersonaPlan.lifeEvent.length > 0) {
            ...
            angular.forEach($rootScope.PersonaPlan.lifeEvent, function (lifeEvent) {
                if (lifeEvent.dream_type_id == 5 && lifeEvent.dependent_reference != null && lifeEvent.dependent_reference != undefined && lifeEvent.dependent_reference.indexOf('child_') == 0) {
                    var findIndex = null;
    ...
    this.init = function () {
            ...
            "graphs": [
              {
                  ...
                  "balloonFunction": function (graphDataItem, graph) {
                      ...
                      var str = utilService.translate("Annual Income") + ": <span style='font-size:11px; color:#000000;'><b> " + $rootScope.PersonaPlan.currency_code + " " + $filter('number')(dataContext.income, 0) + "</b></span>";
                      str += "<br />";
                      str += utilService.translate("Annual Expense") + ": <span style='font-size:11px; color:#000000;'><b> " + $rootScope.PersonaPlan.currency_code + " " + $filter('number')(dataContext.expense, 0) + "</b></span>";
                      return str;
                  }
              }, {
                  id: "expense",
        ...
        $rootScope.summaryCardService.updateSavingRate();
        this.chart = chart;
    ...
    this.getDataByIndex = function (index, status) {
        var data = {
            'MonthlyIncome': {},
            'MonthlyExpense': {},
            'MonthlySaving': {},
        ...
        data.MonthlyIncome.age = $rootScope.PersonaPlan.start_age + index;
        data.MonthlyIncome.income_from_properties = this.getIncomeFrom(index, 'income_from_properties');
        data.MonthlyIncome.income_from_properties = isCumulative_inflation ? data.MonthlyIncome.income_from_properties / $rootScope.Cumulative_inflation[index] : data.MonthlyIncome.income_from_properties;
        data.MonthlyIncome.exceptional_incomes = isCumulative_inflation ? $rootScope.IncomeData.exceptional_incomes[index] / $rootScope.Cumulative_inflation[index] : $rootScope.IncomeData.exceptional_incomes[index];
        data.MonthlyIncome.recurring_incomes = isCumulative_inflation ? $rootScope.IncomeData.recurring_incomes[index] / $rootScope.Cumulative_inflation[index] : $rootScope.IncomeData.recurring_incomes[index];
        data.MonthlyIncome.income_from_social_security = isCumulative_inflation ? $rootScope.IncomeData.income_from_social_security[index] / $rootScope.Cumulative_inflation[index] : $rootScope.IncomeData.income_from_social_security[index];
        data.MonthlyIncome.income_from_salary = isCumulative_inflation ? $rootScope.IncomeData.income_from_salary[index] / $rootScope.Cumulative_inflation[index] : $rootScope.IncomeData.income_from_salary[index];
        data.MonthlyIncome.income = data.MonthlyIncome.income_from_salary + data.MonthlyIncome.income_from_properties + data.MonthlyIncome.exceptional_incomes + data.MonthlyIncome.income_from_social_security;
        data.MonthlyExpense.recurring_expense = isCumulative_inflation ? $rootScope.ExpenseData.recurring_expense[index] / $rootScope.Cumulative_inflation[index] : $rootScope.ExpenseData.recurring_expense[index];
        data.MonthlyExpense.MortageRepayment = isCumulative_inflation ? this.getMortageRepayment(index) / $rootScope.Cumulative_inflation[index] : this.getMortageRepayment(index);
        data.MonthlyExpense.expenses_from_properties = isCumulative_inflation ? this.getExpensesFromProperty(index) / $rootScope.Cumulative_inflation[index] : this.getExpensesFromProperty(index);
        data.MonthlyExpense.exception_expenses = isCumulative_inflation ? $rootScope.ExpenseData.insufficient_funds[index] / $rootScope.Cumulative_inflation[index] : parseFloat($rootScope.ExpenseData.insufficient_funds[index]);
        data.MonthlyExpense.expenses = data.MonthlyExpense.recurring_expense + data.MonthlyExpense.MortageRepayment + data.MonthlyExpense.expenses_from_properties + data.MonthlyExpense.exception_expenses;
        data.MonthlySaving.net_cpf_contribution = $rootScope.net_cpf_contribute[index] * 1000;
        data.MonthlySaving.own_saving = Math.max(0, data.MonthlyIncome.income - data.MonthlyExpense.expenses - data.MonthlySaving.net_cpf_contribution);
        data.MonthlySaving.saving = Math.max(0, data.MonthlySaving.net_cpf_contribution + data.MonthlySaving.own_saving);
        return data;
    }
    this.bindingDataForView = function (index) {
        if ($rootScope.IncomeData.recurring_incomes != null) {
            var data = this.getDataByIndex(index);
            $rootScope.MonthlyIncome = angular.copy(data.MonthlyIncome);
            $rootScope.MonthlyExpense = angular.copy(data.MonthlyExpense);
            $rootScope.MonthlySaving = angular.copy(data.MonthlySaving);
            if ($rootScope.MonthlyIncome.income == 0 || $rootScope.MonthlySaving.own_saving <= 0)
                $rootScope.MonthlySaving.monthlyRate = 0;
            else
                $rootScope.MonthlySaving.monthlyRate = $rootScope.MonthlySaving.saving / $rootScope.MonthlyIncome.income * 100;
            if (index == 0) {
                $rootScope.retirementLifeStyle.expenseAtRetirement = $rootScope.MonthlyExpense.expenses / 12;
                $rootScope.retirementLifeStyle.incomeAtRetirement = $rootScope.MonthlyIncome.income / 12;
            }
            this.updateSavingRateDonutChart();
            $rootScope.utilService.scopeApply('dev_detailSavingRate');
        }
    }
    ...
    this.openRowData = function (name) {
        $("*[parent='" + name + "']").slideToggle('slow');
        $rootScope.timelineService.SendingScreenSharingDataObject(name, 'clickOnSavingRate', 'clickOnSavingRate', '');
    }
    return this;
});
