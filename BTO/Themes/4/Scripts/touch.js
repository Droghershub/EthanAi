(function ($) {
    ...
    Dragging.prototype.move = function (e) {
        ...
        trigger(el, 'dragging:event', e, this);
        ...
        if (this.opts.clone) {
            if (typeof this.opts.clone === 'function') {
                ...
                if (this.opts.cloneClass) {
                    helper.addClass(this.opts.cloneClass).removeClass("btn-float")
                }
            }
            helper.addClass("remove-transition")
          
            var position = this.el.position()
            helper.css('position', 'absolute')
                  .css('left', position.left).css('top', position.top)
                    .css('z-index', 1000)
                  .width(this.el.outerWidth()).height(this.el.outerHeight())
