...
var mouseXFirstOnScreen = 0;
var mouseYFirstOnScreen = 0;

var ItemSelected = 0;
var TimeLineHeight = 0;
var mouseXOnScreen = 0;
var mouseYOnScreen = 0;
var mouseXOnTimeLineMouseDown = 0;
var mouseYOnTimeLineMouseDown = 0;
var mouseXOnTimeLine = 0;
var mouseYOnTimeLine = 0;
var positionXTimeLine = 0;
var positionYTimeLine = 0;
var positionMouseMoved = false;
...
links.Timeline.prototype.repaintFrame = function() {
    ...
    if (!dom.content) {
        ...
        // Add Dom object of chart
        dom.chart = document.createElement("DIV");
        dom.chart.className = "timeline-chart";
        dom.chart.id = dom.container.id + '-timeline-chart';
        dom.frame.appendChild(dom.chart);

        dom.chart_legend = document.createElement("DIV");
        dom.chart_legend.className = "timeline-legend";
        dom.chart_legend.id = dom.container.id + '-timeline-legend';
        dom.frame.appendChild(dom.chart_legend);
    ...
    dom.chart.style.left = size.contentLeft + "px";
    dom.chart.style.top = "0px";
    dom.chart.style.width = size.contentWidth + "px";
    dom.chart.style.height = (size.frameHeight - 12) + "px";
    dom.chart.style.position = "absolute";

    dom.chart_legend.style.top = "10px";
    dom.chart_legend.style.width = size.contentWidth + "px";
    dom.chart_legend.style.position = "absolute";
...
links.Timeline.prototype.repaintAxisHorizontal = function() {
    ...
    if (hasAxis) {
        if (axis.line) {
            ...
            axis.line = line;
            var brokendiv = document.createElement("DIV");
            //brokendiv.id = "timeline-broken-div";
            brokendiv.className = "timeline-broken-div";
            brokendiv.style.position = "absolute";
            brokendiv.style.right =  "0px";
            brokendiv.style.width = "0px";
            brokendiv.style.height = options.heightBroken;
            if (options.renderBrokentext === true) {
                var brokertext = document.createElement("DIV");
                brokertext.id = "timeline-broken-text";
                brokertext.className = "timeline-broken-text";
                brokertext.style.width = options.heightBroken;
                brokendiv.appendChild(brokertext);
            }
            axis.frame.appendChild(brokendiv);
            }
...
links.Timeline.prototype.repaintDeleteButton = function () {
    ...
    if (item && item.rendered && this.isEditable(item) && item.deleteable) {
...
links.Timeline.prototype.onMouseDown = function (event) { 
    isdoubleClick = false;
    positionMouseMoved = false;
    event = event || window.event;

    mouseXFirstOnScreen = links.Timeline.getPageX(event) - $(document).scrollLeft();
    mouseYFirstOnScreen = links.Timeline.getPageY(event) - $(document).scrollTop();


    TimeLineHeight = links.Timeline.getAbsoluteTop(this.dom.content);
    item_selected_id = 0;
    var params = this.eventParams,
        options = this.options,
        dom = this.dom;
    ...
    positionXTimeLine = links.Timeline.getAbsoluteLeft(this.dom.content);
    positionYTimeLine = links.Timeline.getAbsoluteTop(this.dom.content);
    ...
    if (item) {
        item.startbeforemove = item.start;        
        mouseXOnTimeLineMouseDown = links.Timeline.getPageX(event) - item.left - links.Timeline.getAbsoluteLeft(this.dom.content);
        mouseYOnTimeLineMouseDown = links.Timeline.getPageY(event) - item.top - links.Timeline.getAbsoluteTop(this.dom.content);        
    }
    else
    {
        mouseXOnTimeLineMouseDown = 0;
        mouseYOnTimeLineMouseDown = 0;
    }
    ...
    item_selected_id = params.itemIndex;
    this.trigger('mousedownclick');
};
...
links.Timeline.prototype.onMouseMove = function (event) { 
    event = event || window.event;
    mouseXOnScreen = links.Timeline.getPageX(event) - $(document).scrollLeft();
    mouseYOnScreen = links.Timeline.getPageY(event) - $(document).scrollTop();

    if (mouseXFirstOnScreen == mouseXOnScreen && mouseYFirstOnScreen == mouseYOnScreen)
        positionMouseMoved = false;
    else
        positionMouseMoved = true;

    mouseXOnTimeLine = links.Timeline.getPageX(event) - links.Timeline.getAbsoluteLeft(this.dom.content) ;
    mouseYOnTimeLine = links.Timeline.getPageY(event) - links.Timeline.getAbsoluteTop(this.dom.content)  ;
    ...
    if (params.customTime) {
        ...
        if (params.itemDragLeft && options.timeChangeable) {
          ...
          this.trigger('change');
            ...
            // move the item          
            //console.log(mouseXOnTimeLine + '*' + mouseYOnTimeLine + '*' + size.contentWidth + '*' + options.height + '*' + options.eventMarginAxis + '*' + options.eventMargin);
            if (true) {               
                if (item.name == 'dream') {
                    $('#btn-dream-delete').removeClass('hide');
                    $('#btn-dream').addClass('hide');
                }
                else if (item.name == 'life_event') {
                    $('#btn-life-event-delete').removeClass('hide');
                    $('#btn-life-event').addClass('hide');
                }
            }
            
            left = mouseXOnScreen - mouseXOnTimeLineMouseDown - links.Timeline.getAbsoluteLeft(this.dom.content) + 10;
        ...
        if (this.groups.length && !dragging) {
            ...
            if (left_tracking > 0) {
                var text_show = (timeline.items[0].start_age + item.start.getFullYear() - new Date().getFullYear()) + 1;

                this.repaintAxisMinorText(left, text_show);
            }
...
links.Timeline.prototype.onMouseUp = function (event) {    
    ...
    if (params.customTime) {
        ...
        if (params.moved || params.addItem) {
            ...
            else
            {
                ...
                //if (item && mouseXOnScreen != 0) {
                //    if (this.items[params.itemIndex].name == 'dream') {
                //        if (!($('#btn-dream-delete').offset().left <= (mouseXOnScreen + $(document).scrollLeft()) && (mouseXOnScreen + $(document).scrollLeft()) <= ($('#btn-dream-delete').offset().left + $('#btn-dream-delete').outerWidth())
                //                    && $('#btn-dream-delete').offset().top <= (mouseYOnScreen + $(document).scrollTop()) && (mouseYOnScreen + $(document).scrollTop()) <= ($('#btn-dream-delete').offset().top + $('#btn-dream-delete').outerHeight()))) {
                //            this.trigger('changed');
                //        }
                       
                //    }
                //    else if (this.items[params.itemIndex].name == 'life_event') {
                //        if (!($('#btn-life-event-delete').offset().left <= (mouseXOnScreen + $(document).scrollLeft()) && (mouseXOnScreen + $(document).scrollLeft()) <= ($('#btn-life-event-delete').offset().left + $('#btn-life-event-delete').outerWidth())
                //                            && $('#btn-life-event-delete').offset().top <= (mouseYOnScreen + $(document).scrollTop()) && (mouseYOnScreen + $(document).scrollTop()) <= ($('#btn-life-event-delete').offset().top + $('#btn-life-event-delete').outerHeight()))) {
                //            this.trigger('changed');
                //        }                        
                //    }
                //    else
    ...
    //        if ($('#btn-dream-delete').offset().left <= (mouseXOnScreen + $(document).scrollLeft()) && (mouseXOnScreen + $(document).scrollLeft()) <= ($('#btn-dream-delete').offset().left + $('#btn-dream-delete').outerWidth())
    //                    && $('#btn-dream-delete').offset().top <= (mouseYOnScreen + $(document).scrollTop()) && (mouseYOnScreen + $(document).scrollTop()) <= ($('#btn-dream-delete').offset().top + $('#btn-dream-delete').outerHeight())) {
    //            this.confirmDeleteItem(params.itemIndex);
    //        }
    //        $('#btn-dream-delete').addClass('hide');
    //        $('#btn-dream').removeClass('hide');
    //    }
    //    else if (this.items[params.itemIndex].name == 'life_event') {
    //        if ($('#btn-life-event-delete').offset().left <= (mouseXOnScreen + $(document).scrollLeft()) && (mouseXOnScreen + $(document).scrollLeft()) <= ($('#btn-life-event-delete').offset().left + $('#btn-life-event-delete').outerWidth())
    //                            && $('#btn-life-event-delete').offset().top <= (mouseYOnScreen + $(document).scrollTop()) && (mouseYOnScreen + $(document).scrollTop()) <= ($('#btn-life-event-delete').offset().top + $('#btn-life-event-delete').outerHeight())) {
    //            this.confirmDeleteItem(params.itemIndex);
    //        }
    //        $('#btn-life-event-delete').addClass('hide');
    //        $('#btn-life-event').removeClass('hide');
    //    }
    //}
...
links.Timeline.Item = function (data, options) {
    if (data) {
        ...
        this.startbeforemove = data.start;
        this.end = data.end;
...
links.Timeline.ItemBox.prototype.updatePosition = function (timeline) {
    ...
    if (dom) {
        ...
        if (dom.style.cursor == 'move' && timeline.items[ItemSelected].deleteable) {
            dom.style.position = "fixed";

            //dom.style.left = (mouseXOnScreen - mouseXOnTimeLineMouseDown + 12) + "px";
            dom.style.left = (left + positionXTimeLine) + "px";
            dom.style.top = (mouseYOnScreen - mouseYOnTimeLineMouseDown - 5) + "px";

            //line.style.position = "fixed";
            line.style.left = (left) + "px";
            line.style.top = (mouseYOnTimeLine ) + "px";            
            line.style.height = Math.max((axisTop + this.height - mouseYOnTimeLine), 0) + "px";           
...
links.Timeline.prototype.finalItemsPosition = function(items, groupBase, group) {
    ...
    for (i = 0, iMax = groupFinalItems.length; i < iMax; i++) {
        ...
        if (finalItem.item.name != 'broken_age') {           
            var firstindex = 0, endindex = 0;
            if (finalItem.left < 10)
                firstindex = 0;
            else
                firstindex = parseInt(finalItem.left / 10);

            if (finalItem.right < 10)
                endindex = 0;
            else
                endindex = parseInt(finalItem.right / 10);
            for (k = firstindex; k <= endindex; ++k)
                area[k] = area[k] + 1;
        }
    }
    ...
    var margin_try = (160 / (max)) - max * 2;    
    if (this.size.contentHeight > 200) {
        eventMargin = Math.max(margin_try, this.options.eventMargin);
    }
 ...
 */
links.Timeline.StepDate.prototype.CheckExist = function (date) {
    
    if (timeline == null || timeline == undefined)
        return false;
    for (var i = 0; i < timeline.items.length; i++) {
        if (date.getFullYear() == timeline.items[i].start.getFullYear() + 1)
            return true;
    }
    return false;
};
    ...
    switch (this.scale) {
        ...
        case links.Timeline.StepDate.SCALE.YEAR:
            ...
            if (timeline && timeline.items && timeline.items.length > 0)
                text = timeline.items[0].start_age + date.getFullYear() - new Date().getFullYear();
            if (this.CheckExist(date))
                return text.toString();
            if (date.getFullYear() == options.max.getFullYear())                    
                return text.toString();    
            return "";
        default:                                         return "";
    }
