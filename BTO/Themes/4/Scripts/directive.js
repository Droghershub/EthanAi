...
btoApp.directive('materialSelect', ['$timeout', '$interval', '$rootScope', '$filter', function ($timeout, $interval, $rootScope, $filter) {
    ...
    var controller = ['$scope', function ($scope) {
        ...
        $scope.selectItem = function (data) {
            if ($scope.selected_item_id != data.material_item_id) {
                ...
                $scope.selected_item_label = $scope.translateLabel ? $rootScope.utilService.translate(data.material_item_label) : data.material_item_label;
            ...
            if (angular.isDefined($scope.nameID)) {
                ...
                if (angular.isDefined($scope.attr.required)) {
                    if ($scope.modelData == null) {
                        console.log('invalid');
                        $scope.validateClass = 'ng-dirty ng-invalid ng-invalid-required';
        ...
        $scope.validate = function () {
            ...
            if (angular.isDefined($scope.nameID)) {
                var validityCtrl = $scope.ctrl[$scope.nameID];
                if (angular.isDefined(validityCtrl)) {
                    validityCtrl.$setDirty();
                    validityCtrl.$setValidity('required', true);
                    $scope.validateClass = '';
                    if (angular.isDefined($scope.attr.required)) {
                        if ($scope.modelData == null) {
                            ...
                            $timeout(function () {
                                $scope.validateClass = 'ng-dirty ng-invalid ng-invalid-required';
                                //console.log($scope.validateClass);
                            }, 300);
                        }
                    }
                }
        ...
        $scope.searchItems = function () {
            var liList = $('#' + $scope.ID + '-material-select .li-label');
            var startWith = $scope.textsearch;
            $.each(liList, function (index, item) {
                if (item.textContent.toLowerCase().indexOf(startWith.toLowerCase()) != -1) {
        ...
        $scope.keypressData = function (event) {
    ...
    return {
        ...
        template: function (elem, attrs) {
            ...
            result += '<input id="{{ID}}-select-input" type="text" class="select-dropdown" readonly="true" ng-click="clickSelectBox();"  ng-model="selected_item_label" data-activates="{{ID}}-select-options" ng-class="validateClass" />';
            result += '<ul id="{{ID}}-select-options" class="dropdown-content select-dropdown" >';
            result += '<li ' + styledisplay + ' ng-init="textsearch =\'\'  "><span> <input class="form-control input-search-autocomplete" type="text" id="{{ID}}-input-search" ng-model="textsearch" ng-change="searchItems()"  ng-keypress="keypressData($event)" /> </span></li>';
            result += '<li class="waves-effect li-label" ng-class="{\'active\':selected_item_id==item.material_item_id}" ng-repeat="item in items" ng-click="selectItem(item);"><span ng-if="translateLabel">{{$root.translateData[item.material_item_label]}}</span><span ng-if="!translateLabel">{{item.material_item_label}}</span></li>';
            //result += '<li class="waves-effect li-label" ng-class="{\'active\':selected_item_id==item.material_item_id}" ng-repeat="item in items" ng-click="selectItem(item);"><span>{{item.material_item_label}}</span></li>';
            result += '</ul>';
            result += '</div>';
            return result;
        },
        link: function (scope, elem, attr, ctrl) {
                ...
                if (scope.modelData != null) {
                    ...
                    if (angular.isDefined(scope.labelId) && scope.labelId != '') {
                        if (angular.isDefined(scope.modelData[scope.labelId])) {
                            ...
                            scope.selected_item_label = scope.translateLabel ? $rootScope.utilService.translate(scope.notSelectLabel) : scope.notSelectLabel;
                            angular.forEach(scope.items, function (item) {
                                if (item.material_item_id == scope.selected_item_id) {
                                    scope.selected_item_label = scope.translateLabel ? $rootScope.utilService.translate(item.material_item_label) : item.material_item_label;
                                }
                            })
                        }
                    } else {
                        scope.selected_item_label = scope.translateLabel ? $rootScope.utilService.translate(scope.modelData) : scope.modelData;
                    }
                } else {
                    scope.selected_item_label = scope.translateLabel ? $rootScope.utilService.translate(scope.notSelectLabel) : scope.notSelectLabel;
                }
            ...
            scope.$watch('listData', function (newValue) {
                ...
                if (scope.btoDisabledEvent) {
                    ...
                    $timeout(function () {
                        $timeout(function () {
                            text_width = text_width + 50;
                            // console.log(attr.id + '---' + text_width);
                            if (text_width == null) {
...
btoApp.directive('currencyOrPercent', ['$filter', '$locale', '$timeout', function ($filter, $locale, $timeout) {
    var Controller = ['$scope', '$timeout', function ($scope, $timeout) {
        $scope.changeToPercent = function () {
            $scope.isShowCurrency = false;
            $scope.ngModel = $scope.modelValue / $scope.baseValue;
            $scope.ngModel = $scope.updateValueByFraction($scope.ngModel, 8);
            $scope.updateTooltip();
        }

        $scope.changeToCurrency = function () {
            $scope.isShowCurrency = true;
            $scope.ngModel = $scope.modelValue / 100 * $scope.baseValue;
            $scope.ngModel = $scope.updateValueByFraction($scope.ngModel, 8);
            $scope.updateTooltip();
        }

        $scope.updateValueByFraction = function (value, extendFraction) {
            //console.log($scope, value, extendFraction);
            var result = value.toFixed($scope.fraction);
            if (angular.isDefined(extendFraction) && angular.isNumber(extendFraction)) {
                result = value.toFixed($scope.fraction + parseInt(extendFraction));
            }
        ...
        $scope.updateTooltip = function () {
            ...
            $timeout(function () {
                var tooltipElem = $scope.element[0].getElementsByTagName('a');
                $(tooltipElem).tooltip();
            }, 350);
        }

        $scope.onBlur = function () {
            if ($scope.isShowCurrency) {
                $scope.ngModel = $scope.modelValue;
            } else {
                $scope.ngModel = $scope.modelValue / 100;
            }
        }
    }];
    return {
        ...
        link: function (scope, element, attrs, ngModel) {
            ...
            scope.$watch('ngModel', function (newData, oldData) {
                if (!scope.isShowCurrency) {
                    scope.modelValue = parseFloat(newData) * 100;
                } else {
                    scope.modelValue = parseFloat(newData);
                }
                //console.log(scope.modelValue)
                scope.modelValue = scope.updateValueByFraction(scope.modelValue, 8);
                $timeout(function () {
                    scope.$apply();
                    scope.updateTooltip();
                }, 100);
        ...
        template: function (elem, attrs) {
            var template = '<div class="input-group absolute">'
                          + ' <div class="input-group-addon absolute" ng-click="changeToPercent();" ng-if="isShowCurrency"><a title="{{$root.translateData[\'Switch to percent\']}}" data-placement="bottom" >{{currencySymbol}}</a></div>'
                          + ' <input type="text" class="form-control" ng-class="{\'input-percent\': !isShowCurrency}" name="{{inputName}}" ng-blur="onBlur()" ng-model="modelValue" type="text" ng-currency currency-symbol="" fraction="fraction">'
                          + ' <div class="input-group-addon percent" ng-click="changeToCurrency();"  ng-if="!isShowCurrency"><a title="{{$root.translateData[\'Switch to\']}} {{currencySymbol}}" data-placement="bottom" >&nbsp;%&nbsp;</a></div>'
                          + ' </div>';
            return template;
        }
    }
