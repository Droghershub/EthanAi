...
btoApp.factory('GaugeChart', ['$document', '$timeout', '$compile', '$window', '$filter', '$rootScope', function ($document, $timeout, $compile, $window, $filter, $rootScope) {
    var GaugeChart = function (scope, gaugeChartElem, attributes) {
        ...
        this.isInitGaugeChart = true;
        var self = this;
        $timeout(function () {
            self.isInitGaugeChart = false;
        }, 500);
        this.init();
    }
    ...
    GaugeChart.prototype = {
        ...
        callOnChange: function () {
            //console.log('gauge modelData: ', this.modelData);
            this.hideTooltip();
            if (this.scope.changeValue && !this.isInitGaugeChart) {
...
btoApp.directive('btoGaugeChart', ['GaugeChart', '$compile', function (GaugeChart, $compile) {
    ...
    var controller = ['$scope', function ($scope) {
        $scope.onChangeModelData = function () {           
           
            var modelData = parseFloat($scope.modelData);
            if (!isNaN(modelData)) {
                if (modelData >= 0 && modelData <= $scope.configData.max) {
                    $scope.gaugeChart.setValue(modelData);
                    $scope.gaugeChart.modelData = modelData;
                    $scope.gaugeChart.callOnChange();
                } else if (modelData > $scope.configData.max) {
                    $scope.gaugeChart.modelData = $scope.configData.max;
                    $scope.gaugeChart.setValue($scope.configData.max);
                    $scope.gaugeChart.callOnChange();
                } else {
                    $scope.gaugeChart.modelData = 0;
                    $scope.gaugeChart.setValue(0);
                    $scope.gaugeChart.callOnChange();
                }
            } else {
                $scope.gaugeChart.modelData = 0;
    ...
    return {
        ...
        template: function (elem, attrs) {
            var template = '<div id="' + attrs.id + '" class="bto-gauge">'
                ...
                + '<div class="bto-gauge__value bto-gauge__block"><input class="form-control" ng-disabled="btoDisabledEvent" ng-keyup="onChangeModelData();" ng-model="modelData" max="configData.max" min="0" /></div>'
                + '</div>'
            return template;
        },
        ...
        link: function (scope, elem, attr) {
            ...
            scope.gaugeChart = gaugeChart;
            scope.$watch('modelData', function (newValue) {
                scope.gaugeChart.setValue(newValue);
            });
            return gaugeChart;
        }
