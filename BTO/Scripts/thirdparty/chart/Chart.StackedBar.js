(function (factory) {
	...
	Chart.Type.extend({
		...
		initialize:  function(data){
			...
			var labelsx = [], lengthlb = data.labels.length;
			if (lengthlb > 25) {
			    for (var i = 0; i < lengthlb; i++) {
			        if (i % this.options.sizeOfScaleForRenderXLabel == 0)
			            labelsx[i] = data.labels[i];
			        else
			            labelsx[i] = '';
			    } 
			    this.buildScale(labelsx);
			} else this.buildScale(data.labels);
