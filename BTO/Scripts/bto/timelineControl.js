function CalculateAgeObjectList($rootScope) {    
    ...
    if ($rootScope.profile != null && $rootScope.profile.spouse != null && $rootScope.profile.spouse.age != null) {
...
function initTimeLineItem(canvas, $rootScope, year, name, isRetire, isMovable, imagePath, type, index, dataid, ultilService, $timeout) {
    ...
    item.bind('dblclick dbltap', function () {
        ...
        if (type == 'dream') {
            ...
            ultilService.updateSelectedDreamType('dream', true);
            ...
            ultilService.updateSelectedDreamType('life_event', true);
...
function callToAddDream($rootScope, ultilService) {
    ultilService.AddDream($rootScope, $rootScope.selectedDream);
...
function callToAddLifeEvent($rootScope, ultilService) {
    ultilService.AddLifeEvent($rootScope, $rootScope.selectedLifeEvent);
...
btoApp.service('timelineService', function ($rootScope, personaplanService, ultilService, $cookies, $timeout, $state, $locale, zoomService) {
    this.timelineObj = null;
    var self = this;
    tlService = this;
    function initTimline() {
        ...
        var dreamsdragOption = {
            ...
            start: function () {
                ...
                if (checkMousePositionInTimeLineControl(canvas, $rootScope)) {
                    ultilService.updateDefaultValueOfDream($rootScope);
                    ...
                    if (this.kind == 'dreamevents') {
                        ...
                        ultilService.updateSelectedDreamType('dream');
                        $timeout(function () { 
                            $('#dreamdialog').modal({ backdrop: 'static', keyboard: false });
                            var obj = {
                                datatype: 'dream',
                            ...
                            $('#SaveDream').bind('click', function () {
                                ...
                                $timeout(function () {
                                    ...
                                    callToAddDream($rootScope, ultilService);
                                    if ($rootScope.selectedDream.id == -1) {                                        
                                        return;
                                    }
                                    ...
                                    var newItem = initTimeLineItem(canvas, $rootScope, year, 'dream_' + newid, false, true, image, 'dream', index, newid, ultilService, $timeout);
                        ...
                        ultilService.updateSelectedDreamType('life_event');
                        $timeout(function () { 
                            $('#lifeeventdialog').modal({ backdrop: 'static', keyboard: false });
                            var obj = {
                                datatype: 'lifeevent',
                            ...
                            $('#SaveLifeEvent').bind('click', function () {
                                ...
                                $timeout(function () {
                                    ...
                                    callToAddLifeEvent($rootScope, ultilService);
                                    if ($rootScope.selectedLifeEvent.id == -1) {
                                        return;
                                    }
                                    ...
                                    var newItem = initTimeLineItem(canvas, $rootScope, year, 'lifeevent_' + newid, false, true, image, 'lifeevent', index, newid, ultilService, $timeout);
                                    $rootScope.Settings.isAjaxInstantRequest = false;
                                    requestSaveAndUpdateScreen();
                                    newItem.dragAndDrop(dragOptions);
                                    canvas.addChild(newItem);
                        ...
                        if (index > -1) {
                            ...
                            if (lifeevent == null) {
                                ...
                                if (parseFloat(this.children[0].text) > lifeevent.starting_age) {
                                    ultilService.showWarningMessage(ultilService.translate('You can\'t sell a house that you haven\'t bought yet.'));
                        ...
                        if (datatype == 'dream') {
                            ...
                            if (dream_id_delete != null && dream_id_delete > 0) {//find life event delete also
                                ...
                                if (lifeEvent == null) {
                                    ...
                                    $rootScope.MessageDelete = ultilService.translate("Life Event: {{name}} will be delete too, Are you sure?", { name: lifeEvent.name });
                                }                                
                            }
                        }
                        else {
                        ...
                        $('#OkDelete').bind('click', function () {
                            ...
                            { 
                                if (lifeevent_id_delete != null) {
                                    ultilService.RemoveLifeEvent(lifeevent_id_delete);
                                ...
                                ultilService.RemoveDream(dream_id_delete);
        ...
        var currentButton = InitButtonObject(canvas, c.width * 20 / 100, 30, ultilService.translate('Current Plan'), '#31b0d5', $rootScope, $timeout);
        currentButton.bind('click tap', function () {
            if ($rootScope.PersonaPlan.status != 0) // not a current plan
            {
                ...
                this.switchToCurrentPlan();
            }
        });
        canvas.addChild(currentButton);
        $rootScope.switchToCurrentPlan = function () {
            ...
            InsertDreamAndLifeEvent(canvas, $rootScope, dragOptions, ultilService, $timeout);
            $rootScope.changeCurrency($rootScope.PersonaPlan.currency_code);
            OrderCanvasItem($rootScope, canvas);
            $rootScope.ReuseSocialSecurityAge = $rootScope.PersonaPlan.social_security_age;
            ...
            requestSaveAndUpdateScreen();
            reRenderTimelineObject(canvas, $rootScope);
            ultilService.showSuccessMessage(ultilService.translate('Switched to current plan'));
            personaplanService.changeToCurrentPlan($rootScope.PersonaPlan);
        ...
        $rootScope.switchToNewPlanShare = function (dataObj) {
            ...
            reRenderTimelineObject(canvas, $rootScope);
            ultilService.showSuccessMessage(ultilService.translate('Switched to new plan'));
        };
        $rootScope.switchToNewPlan = function () {
            ...
            reRenderTimelineObject(canvas, $rootScope);
            ultilService.showSuccessMessage(ultilService.translate('Switched to new plan'));
            personaplanService.changeToNewPlan($rootScope.PersonaPlan);
            $rootScope.SendingScreenSharingDataObject($rootScope.PersonaPlan, 'switch', 'newPlan', 'confirmdialog');
        };
        var newButton = InitButtonObject(canvas, c.width * 35 / 100, 30, ultilService.translate('My New Plan'), '#337ab7', $rootScope, $timeout);
        ...
        var saveScenariosButton = InitButtonObject(canvas, c.width * 50 / 100, 30, ultilService.translate('Save Scenario'), '#337ab7', $rootScope, $timeout);
        ...
        */
        ...
        $rootScope.addLifeEventToPlayBack = function (dataObj) { 
         ...
         $rootScope.resetPlanForPlayBack_open = function (dataObj) {
             $timeout(function () {
                 ...
                 $rootScope.confirmdialog.title = ultilService.translate('Reset Plan');
                 $rootScope.confirmdialog.content = '';
                 if ($rootScope.PersonaPlan.status == 0)
                     $rootScope.confirmdialog.messenger = ultilService.translate('Do you want to reset current plan?');
                 else if ($rootScope.PersonaPlan.status == 1)
                     $rootScope.confirmdialog.messenger = ultilService.translate('Do you want to reset new plan?');
                 $rootScope.confirmdialog.OkText = ultilService.translate('Ok');
                 $rootScope.confirmdialog.cancelText = ultilService.translate('Cancel');
        ...
        $rootScope.resetPlanForPlayBack_ok = function (dataObj) {
            $timeout(function () {
                ...
                $rootScope.ReuseSocialSecurityAge = $rootScope.PersonaPlan.social_security_age;
                if ($rootScope.PersonaPlan.status == 0)
                    ultilService.showSuccessMessage(ultilService.translate('Current plan has been reset.'));
                else if ($rootScope.PersonaPlan.status == 1)
                    ultilService.showSuccessMessage(ultilService.translate('Resetted new plan'));
                // ultilService.LoadDataByUserForShareScreen(dataObj.newValue.user_id);
        ...
        $rootScope.addDreamToPlayBack_open = function (dataObj) {
         ...
         $rootScope.addDreamToPlayBack_ok = function (dataObj) {
             ...
             $timeout(function () {
                 ...
                 var newItem = initTimeLineItem(canvas, $rootScope, year, dataObj.type + '_' + newid, false, true, image, dataObj.type, index, newid, ultilService, $timeout);
                 canvas.addChild(newItem);
             }, 1000);
             $timeout(function () {
        ...
        $rootScope.addDreamToPlayBackward = function (dataObj) {
            ultilService.updateDefaultValueOfDream($rootScope);
            $rootScope.isEditDream = false;
            ultilService.updateSelectedDreamType(dataObj.type);
            $rootScope.PersonaPlan.dreams[$rootScope.PersonaPlan.dreams.length] = dataObj.newValue;
            bindDreamtoModalDialog($rootScope, $rootScope.PersonaPlan.dreams.length - 1);
            var image = 'Content/images/';
            var index = dataObj.newValue.index;
        ...
        $rootScope.editDreamToPlayBack_open = function (dataObj) {
            ...
            ultilService.updateSelectedDreamType(dataObj.type, true);
            $timeout(function () {
                bindDreamtoModalDialog($rootScope, index, dataObj.newValue);
                $('#dreamdialog').modal({ backdrop: 'static', keyboard: false });
        ...
        */
        ...
        $rootScope.moveCanvasObject = function (name, fromYear, toYear, moveSpeed) {
            var duration = 2000;
            var obj = $rootScope.findCanvasObjectByName(name);
            ...
            if ($rootScope.PersonaPlan.death_age != null && broken_age != null) {
                if ($rootScope.old_broken_age != broken_age) {
                    if (broken_age <= $rootScope.PersonaPlan.death_age) {
                        ...
                        brokenPig.children[0].text = ultilService.translate('Congratulations {{name}}! \nYour goals are all achievable.', { name: $rootScope.profile.client.first_name });
    ...
    this.changeTextOfTimeline = function () {
        ...
        try {
            ...
            var currentPlanText = ultilService.translate("Current Plan");
            ...
            var myNewPlanText = ultilService.translate("My New Plan");
            ...
            var saveScenarioText = ultilService.translate("Save Scenario");
            ...
            if (angular.isDefined($rootScope.PersonaPlan) && angular.isDefined($rootScope.PersonaPlan.death_age) && angular.isDefined($rootScope.MainResult) && angular.isDefined($rootScope.MainResult.broken_age)) {
                if ($rootScope.MainResult.broken_age > $rootScope.PersonaPlan.death_age) {
                    var goalArchiveText = ultilService.translate('Congratulations {{name}}! \nYour goals are all achievable.', { name: $rootScope.profile.client.first_name });
                }
            }
    ...
    function InsertDreamAndLifeEvent(canvas, $rootScope, dragOptions, ultilService, $timeout) {
        var items = [];
        var i = 0;
        angular.forEach($rootScope.PersonaPlan.dreams, function (dream) {

            items[i] = initTimeLineItem(canvas, $rootScope, dream.purchase_age, 'dream_' + dream.id, false, true, "Content/images/" + dream.dream_type.image_name, 'dream', i, dream.id, ultilService, $timeout);
        ...
        angular.forEach($rootScope.PersonaPlan.lifeEvent, function (lifes) {
            ...
            items[i] = initTimeLineItem(canvas, $rootScope, lifes.starting_age, 'lifeevent_' + lifes.id, false, true, "Content/images/" + lifes.dream_type.image_name, 'lifeevent', j, lifes.id, ultilService, $timeout);
            items[i].dragAndDrop(dragOptions);
    ...
    function resetUserPlan() {
        ...
        personaplanService.resetUserPlan($rootScope.PersonaPlan.user_id, $rootScope.PersonaPlan.status).then(
                function (obj) {
                    if (obj != null) {
                        $rootScope.old_broken_age = 0;
                        $rootScope.PersonaPlan = obj.data.plan;                        
                        $rootScope.salaryEvolutionAndInflaction.salaryEvolution = $rootScope.PersonaPlan.salary_evolution * 100;
                        $rootScope.salaryEvolutionAndInflaction.inflaction = $rootScope.PersonaPlan.inflation * 100;
                        zoomService.resetZoomData();
                        $rootScope.zoomData.isShow = false;
                        ...
                        if ($rootScope.PersonaPlan.status == 0)
                            ...
                            ultilService.showSuccessMessage(ultilService.translate('Resetted new plan'));
                        $rootScope.SendingScreenSharingDataObject(obj.data, 'resetplan', 'ok', 'confirmdialog');
                    }
                }
        ); 
    ...
    function calculateData() {
        ...
        if (checkCalculatorCurrentRequest()) { 
            if ($rootScope.tabType == 'IncomeExpense') { 
                (requestCalculator = personaplanService.calculateIncomeAndExpense($rootScope.PersonaPlan)).then(
                     function (obj) {
                         if (obj != null) {
                             $rootScope.MainResult = obj.basic;
                             $rootScope.IncomeExpenseChart = obj;
                             repaintchart();
                             canSaveAndUpdateScreen();
                             var item = {
                                 PersonaPlan: $rootScope.PersonaPlan,
                                 basic: obj
                             };
                             $rootScope.SendingScreenSharingDataObject(item, 'tab', 'change', 'IncomeExpenses');
                         }
                     },
            ...
            } else if ($rootScope.tabType == 'liquidIlliquidAsset') {
                (requestCalculator = personaplanService.calculateEquityCurve($rootScope.PersonaPlan)).then(
                     function (obj) {
                         if (obj != null) {
                             $rootScope.MainResult = obj.basic;
                             ...
                             $rootScope.SendingScreenSharingDataObject(item, 'tab', 'change', 'LiquidIlliquidAsset');
                         }
                     },
                     function (errorMessage) {
                         $rootScope.isLoading = false;
            ...
            } else if ($rootScope.tabType == 'IlliquidAsset') {
                (requestCalculator = personaplanService.calculateIlliquidCurve($rootScope.PersonaPlan)).then(
                     function (obj) {
                         if (obj != null) {
                             $rootScope.MainResult = obj.basic;
                             ...
                             $rootScope.SendingScreenSharingDataObject(item, 'tab', 'change', 'IlliquidAsset');
                         }
                     },
                     function (errorMessage) {
                         $rootScope.isLoading = false;
                ...
                if ($rootScope.tabType == 'Ranking_Dream') {
                    (requestCalculator = personaplanService.calculateResult($rootScope.PersonaPlan)).then(
                    function (obj) {
                        if (obj != null) {
                            ...
                            $rootScope.SendingScreenSharingDataObject(item, 'tab', 'change', 'RankingDreams');
                        }
                    },
                    function (errorMessage) {
                        $rootScope.isLoading = false;
                    ...
                    (requestCalculator = personaplanService.calculateResult($rootScope.PersonaPlan)).then(
                     function (obj) {
                         if (obj != null) {
                             ...
                             $rootScope.SendingScreenSharingDataObject(item, 'tab', 'change', 'Main');
                         }
                     },
                     function (errorMessage) {
                         $rootScope.isLoading = false;
    ...
    function updateData() {
        var isplayback = $rootScope.isStartingPlayback == true ? true : false;
        if ($rootScope.PersonaPlan && $rootScope.PersonaPlan.id > 0) {
            if (checkCurrentRequest() && isplayback == false) {
                (requestToServer = personaplanService.updatePersonaPlan($rootScope.PersonaPlan)).then(
                   function (data) {
    ...
    function canRequestUpdate(obj) {
        ...
        if (utils.compareTimeWithDelay($rootScope.Settings.timeBefore, $rootScope.Settings.timeNow, $rootScope.Settings.timerDelay) || $rootScope.Settings.isInstantRequest == true) {
    ...
    this.addSpouseRetirementIcon = function (year) {
        var retirementPics = initTimeLineItem(this.timelineObj.canvas, $rootScope, year, 'RetirementAgeOfSpouse', true, true, "Content/images/spouse_retire.png", "retirement");
        //retirementPics.dragAndDrop(dragOptions);
        this.timelineObj.canvas.addChild(retirementPics);
        this.spouseRetirementPic = retirementPics;
    }

    this.removeSpouseRetirementIcon = function () {
        if (this.spouseRetirementPic) {
            this.timelineObj.canvas.removeChild(this.spouseRetirementPic)
        }
    }
    ...
    $rootScope.UpdateControlForShareScreen = function (obj) { 
        switch (obj.action) {
            ...
            case 'changedropdownlist':
                switch (obj.actionEvent) {
                    case 'changed':
                        utils.addHighLightClassforDirectId('selectDreamOrEvent');
                        $rootScope.timelineControl.selectedItemOnTimelineId = obj.newValue;
                        $rootScope.selectDreamOrLifeEvent();
                        $timeout(function () {
                            utils.removeHighLightClassforDirectId('selectDreamOrEvent');
                        }, 1500);
                        break;
                }
                break;
            ...
            case 'tab':
                ...
                if (obj.clientActionObjectEvent == 'begin') {
                    if (obj.clientActionObjectId == 'incomeExpensebar' || obj.clientActionObjectId == 'expenseAtRetirement') {
                        utils.addHighLightClassforProgressbar(obj.clientActionObjectId);
                        $timeout(function () {
                    ...
                    utils.addHighLightClassforDirectId(controlId);
                    $timeout(function () {
                        utils.removeHighLightClassforDirectId(controlId);
                    }, 2000);

                } else {
                    if (obj.clientActionObjectId == 'incomeExpensebar' || obj.clientActionObjectId == 'expenseAtRetirement') {
                        utils.removeHighLightClassforProgressbar(obj.clientActionObjectId);
                    }
                    else {
                        utils.removeHighLightClassforDirectId(obj.clientActionObjectId);
                    }
